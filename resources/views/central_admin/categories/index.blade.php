<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight flex items-center justify-between">
            ...
            <a href="{{ route('central_admin.categories.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-sm transition text-sm">
    ...
    <div class="py-8 bg-gray-50/50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-real-time-filter table-id="categoriesTable" placeholder="ค้นหาชื่อหมวดหมู่..." count-label="หมวดหมู่">
            </x-real-time-filter>

            <div class="bg-white md:overflow-hidden shadow-sm rounded-3xl border border-gray-100 md:overflow-x-auto">
                <table id="categoriesTable" class="w-full text-left border-collapse block md:table">
                    <thead class="hidden md:table-header-group">
                        <tr class="border-b border-gray-100 text-slate-500 text-xs font-semibold uppercase bg-slate-50/80">
                            ...
                            <th class="py-4 px-5">ชื่อหมวดหมู่</th>
                    ...
                    <tbody class="block md:table-row-group divide-y-0 md:divide-y divide-gray-50 bg-transparent md:bg-white space-y-4 md:space-y-0 p-4 md:p-0">
                        ...
                        <tr class="hover:bg-indigo-50/30 transition-colors block md:table-row bg-white border border-gray-100 md:border-0 rounded-2xl md:rounded-none shadow-sm md:shadow-none p-4 md:p-0 relative"
                            ...
                            <td class="block md:table-cell py-3 md:py-4 md:px-5 text-center border-b border-gray-50 md:border-0" :class="expanded ? 'block' : 'hidden md:table-cell'">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('central_admin.categories.edit', $item) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition w-full md:w-auto justify-center">
                                        <i class="fa-solid fa-pen-to-square"></i> แก้ไข
                                    </a>
                                    <form action="{{ route('central_admin.categories.destroy', $item) }}" method="POST" class="inline w-full md:w-auto" onsubmit="return confirm('ยืนยันการลบหมวดหมู่ {{ $item->name }}?')">
                                        @csrf @method("DELETE")
                        ...
                        <tr class="block md:table-row">
                            <td colspan="3" class="block md:table-cell py-14 text-center text-slate-400">
                                <i class="fa-solid fa-folder-open text-4xl mb-2 block text-slate-300"></i>ยังไม่มีหมวดหมู่ในระบบ
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
