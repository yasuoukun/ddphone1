            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100 p-8">
                <form action="{{ route('central_admin.coupons.store') }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 font-semibold" placeholder="ระบุชื่อโปรโมชันหรือคูปอง">
                    </div>

                    <div class="mb-5">
                        <label class="block text-sm font-bold text-slate-700 mb-2">รหัสโค้ดคูปอง (ตัวพิมพ์ใหญ่ เช่น DISCOUNT50)</label>
                        <input type="text" name="code" value="{{ old('code') }}" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 font-mono font-bold text-indigo-600 uppercase" required placeholder="เช่น SUMMER100">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        <div>
                            ...
                            <input type="number" step="0.01" name="discount_amount" value="{{ old('discount_amount') }}" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 font-bold text-rose-600 text-lg" required min="0" placeholder="เช่น 500.00">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">วันหมดอายุ (Expires At)</label>
                            <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 font-semibold" required min="{{ date('Y-m-d') }}">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                        ...
                        <button type="submit" class="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition shadow-sm">บันทึกคูปองส่วนลด</button>
                    </div>
                </form>
            </div>
        </div>
